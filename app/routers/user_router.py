...
async def get_users(db: Session = Depends(get_db), authed=Security(is_authenticated, scopes=[Scopes.ADMIN.value])):
...
async def create_user(new_user: UserCreate, db: Session = Depends(get_db),
                      authed: bool = Security(is_authenticated, scopes=[Scopes.GUEST.value])):
    user_service.start_session(db)
    return user_service.create_user(new_user)


@router.get('/by', response_model=UserScheme)
async def get_user_by(db: Session = Depends(get_db), authed=Security(is_authenticated, scopes=[Scopes.GUEST.value]),
                      user_id: int = 0, email: str = '', phone_number: str = ''):
    user_service.start_session(db)
    if user_id:
...
async def get_user_purchases(user_id, db: Session = Depends(get_db),
                             authed=Security(is_authenticated, scopes=[Scopes.USER.value])):
    user_service.start_session(db)
